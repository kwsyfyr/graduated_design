...
model = Sequential()
model.add(SimpleRNN(40, input_shape=(X_train.shape[1], X_train.shape[2])))
...
plot_model(model, to_file='./model_SimpleRNN.png', show_shapes=True)

# history = model.fit(X_train, Y_train, epochs=300, batch_size=20, validation_data=(x_test, y_test), verbose=1)
history = model.fit(X_train, Y_train, epochs=600, batch_size=20, verbose=1)
